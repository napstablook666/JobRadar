...
import { Save, KeyRound, ExternalLink, Code2, Info, RefreshCw } from 'lucide-react'
...
export default function EnvConfig() {
  ...
  const modelOptions = useMemo(() => {
    const set = new Set(modelList.filter(Boolean))
    if (envConfig.model?.trim()) {
      set.add(envConfig.model.trim())
    }
    return Array.from(set).sort((a, b) => a.localeCompare(b))
  }, [modelList, envConfig.model])
  ...
  // 从 OpenAI 兼容 /v1/models 拉取可用模型列表（优先使用当前表单中的 baseUrl/apiKey）
  const fetchModels = async () => {
    if (!envConfig.baseUrl?.trim()) {
      setModelFetchMsg('请先填写 API Base URL')
      return
    }
    if (!envConfig.apiKey?.trim()) {
      setModelFetchMsg('请先填写 API Key')
      return
    }
    ...
    try {
      ...
      if (!response.ok || !result?.success) {
        throw new Error(result?.message || `获取模型列表失败 (${response.status})`)
      }

      const models: string[] = Array.isArray(result.data)
        ? result.data.map((m: unknown) => String(m)).filter(Boolean)
        : []

      setModelList(models)

      if (models.length === 0) {
        setModelFetchMsg('接口返回成功，但未解析到任何模型')
      } else {
        setModelFetchMsg(`已获取 ${models.length} 个模型`)
        // 当前模型不在列表中时，默认选第一个
        if (!envConfig.model?.trim() || !models.includes(envConfig.model.trim())) {
          setEnvConfig((prev) => ({ ...prev, model: models[0] }))
        }
      }
    } catch (error) {
      console.error('获取模型列表失败:', error)
      setModelFetchMsg(error instanceof Error ? error.message : '获取模型列表失败')
    } finally {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  >
                    <RefreshCw className={`mr-1 ${loadingModels ? 'animate-spin' : ''}`} />
                ...
                {modelList.length > 0 ? (
                  ...
                  <Input
                    ...
                    value={envConfig.model}
                    onChange={(e) => setEnvConfig({ ...envConfig, model: e.target.value })}
                    placeholder="gpt-5-nano-2025-08-07 或点击获取模型列表"
                  />
                )}

                {/* 已获取列表时仍允许手动输入未出现在列表中的模型名 */}
                {modelList.length > 0 && (
                  <Input
                    type="text"
                    value={envConfig.model}
                ...
                <p className="text-xs text-muted-foreground">
                  使用的AI模型名称
                  {modelFetchMsg ? (
                    <span className={`ml-2 ${modelFetchMsg.includes('失败') || modelFetchMsg.includes('请先') ? 'text-red-500' : 'text-emerald-600'}`}>
                      · {modelFetchMsg}
                    </span>
                  ) : null}
                </p>
