...
import { Briefcase, Save, Search, Banknote, Building2, Trash2, Plus, Play, Square, LogOut, ChevronDown } from 'lucide-react'
      ...
      <div className="space-y-6 mt-6">
          ...
          <Card className="animate-in fade-in slide-in-from-bottom-4 duration-700">
            <CardHeader>
              ...
              <CardDescription>控制岗位筛选、自动招呼语和图片简历发送</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="space-y-2">
                  <Label htmlFor="enableAiScreening">AI岗位筛选</Label>
                  <Select
                    id="enableAiScreening"
                    value={String(config.enableAiScreening ?? 0)}
                    onChange={(e) => setConfig({ ...config, enableAiScreening: Number(e.target.value) })}
                  >
                    <option value="0">关闭</option>
                    <option value="1">开启</option>
                  </Select>
                  <p className="text-xs text-muted-foreground">开启后先评分，未通过岗位自动跳过</p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="aiMinScore">AI最低分数</Label>
                  <Input
                    id="aiMinScore"
                    type="number"
                    min={0}
                  ...
                  />
                  <p className="text-xs text-muted-foreground">默认 70 分</p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="enableAi">AI生成招呼语</Label>
                  <Select
                    id="enableAi"
                    value={String(config.enableAi ?? 0)}
                    onChange={(e) => setConfig({ ...config, enableAi: Number(e.target.value) })}
                  >
                    <option value="0">关闭</option>
                    <option value="1">开启</option>
                  </Select>
                  <p className="text-xs text-muted-foreground">失败时回退到下方默认招呼语</p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="sendImgResume">自动发送图片简历</Label>
                  <Select
                    id="sendImgResume"
                    value={String(config.sendImgResume ?? 0)}
                    onChange={(e) => setConfig({ ...config, sendImgResume: Number(e.target.value) })}
                  >
                    <option value="0">关闭</option>
                    <option value="1">开启</option>
                  </Select>
                  <p className="text-xs text-muted-foreground">使用 resources/resume.jpg</p>
                </div>
              </div>

              <div className="space-y-2 mt-6">
                <Label htmlFor="sayHi">默认招呼语</Label>
                <Textarea
                  id="sayHi"
                  value={config.sayHi ?? ''}
                  onChange={(e) => setConfig({ ...config, sayHi: e.target.value })}
                  placeholder="AI生成失败时使用的招呼语"
                  rows={3}
                />
              </div>
            </CardContent>
