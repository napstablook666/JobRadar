...
import { RefreshCw, Download, ChartBar, LineChart, Briefcase } from "lucide-react"
...
function badgeClass(value?: string) {
  ...
  if (status.includes("已投递")) return `${base} bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300`
  if (status.includes("已过滤")) return `${base} bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-300`
  if (status.includes("失败")) return `${base} bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300`
...
export default function AnalysisContent({ showHeader = false }: { showHeader?: boolean }) {
  ...
  return (
    <div className="space-y-8">
      {showHeader && (
        <PageHeader title="智联 投递分析" subtitle="基于 zhilian_data 表的统计图与列表分析" icon={<ChartBar size={28} />} />
      )}
        ...
        <CardContent>
          ...
          <div className="mt-4 flex items-center gap-3">
            <Button variant="default" onClick={() => loadStats()} disabled={loadingStats}>
              <RefreshCw className="mr-1" /> 刷新统计
            </Button>
            <Button variant="outline" onClick={() => loadList(1, size)}>
              <Briefcase className="mr-1" /> 刷新列表
            </Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2"><ChartBar /> 投递状态分布</CardTitle>
            <CardDescription>按 delivery_status 聚合</CardDescription>
          </CardHeader>
          <CardContent>
            {stats ? (
              <ChartSurface type="pie" labels={stats.charts.byStatus.map((x) => x.name)} data={stats.charts.byStatus.map((x) => x.value)} colors={CATEGORY_COLORS} />
            ) : (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2"><ChartBar /> 城市TOP10</CardTitle>
            <CardDescription>按地区聚合</CardDescription>
          </CardHeader>
          <CardContent>
            {stats ? (
              <ChartSurface type="bar" labels={stats.charts.byCity.map((x) => x.name)} data={stats.charts.byCity.map((x) => x.value)} color="#3b82f6" />
            ) : (
              <div className="text-muted-foreground">加载中...</div>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2"><ChartBar /> 公司岗位数TOP10</CardTitle>
            <CardDescription>按公司名称聚合</CardDescription>
          </CardHeader>
          <CardContent>
            {stats ? (
              <ChartSurface type="bar" labels={stats.charts.byCompany.map((x) => x.name)} data={stats.charts.byCompany.map((x) => x.value)} color="#10b981" />
            ) : (
              <div className="text-muted-foreground">加载中...</div>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2"><ChartBar /> 经验分布</CardTitle>
            <CardDescription>不同经验要求的岗位数</CardDescription>
          </CardHeader>
          <CardContent>
            {stats ? (
              <ChartSurface type="bar" labels={stats.charts.byExperience.map((x) => x.name)} data={stats.charts.byExperience.map((x) => x.value)} color="#f59e0b" />
            ) : (
              <div className="text-muted-foreground">加载中...</div>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2"><ChartBar /> 学历分布</CardTitle>
            <CardDescription>不同学历要求的岗位数</CardDescription>
          </CardHeader>
          <CardContent>
            {stats ? (
              <ChartSurface type="bar" labels={stats.charts.byDegree.map((x) => x.name)} data={stats.charts.byDegree.map((x) => x.value)} color="#6366f1" />
            ) : (
              <div className="text-muted-foreground">加载中...</div>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2"><LineChart /> 薪资区间分布</CardTitle>
            <CardDescription>基于中位数K的桶聚合（后端或前端计算）</CardDescription>
          </CardHeader>
          <CardContent>
            {stats ? (
              <ChartSurface type="line" labels={(computedSalaryBuckets.length ? computedSalaryBuckets : stats.charts.salaryBuckets).map((x) => x.bucket)} data={(computedSalaryBuckets.length ? computedSalaryBuckets : stats.charts.salaryBuckets).map((x) => x.value)} color="#ef4444" />
            ) : (
              <div className="text-muted-foreground">加载中...</div>
            )}
          </CardContent>
                ...
                {items.map((it, idx) => (
                  <tr key={`${it.jobId}-${idx}`} className="border-t">
                    ...
                    <td className="py-2 px-3 whitespace-nowrap">
                      <span className={badgeClass(it.deliveryStatus)}>{it.deliveryStatus || ""}</span>
                    </td>
                    <td className="py-2 px-3 whitespace-nowrap">
                      {it.jobLink ? (
