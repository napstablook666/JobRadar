...
import { RefreshCw, Download, ChartBar, LineChart, PieChart, Briefcase, ExternalLink, CheckCircle2 } from "lucide-react"
...
type Job51Item = {
  ...
  externalApplyStatus?: string
  externalApplyTime?: string
}
...
export default function AnalysisContent({ showHeader = false }:{ showHeader?: boolean }) {
  ...
  const [markingExternalId,setMarkingExternalId]=useState<number|null>(null)

  const statusOptions = ["未投递","已投递","待外部申请","外部已申请"]

  useEffect(()=>{
    void loadStats()
    // eslint-disable-next-line react-hooks/exhaustive-deps -- initial stats fetch only
  },[])
  ...
  const markExternalApplied = async (jobId:number)=>{
    try{
      setMarkingExternalId(jobId)
      const res = await fetch(`${API_BASE}/api/51job/external-apply/${jobId}`, { method:"POST" })
      const data = await res.json() as { success?: boolean; message?: string }
      if (!res.ok || !data.success) throw new Error(data.message || "标记失败")
      await loadList(page, size)
      await loadStats()
    }catch(e){
      console.error("mark external apply failed",e)
      alert(e instanceof Error ? e.message : "标记失败，请稍后重试")
    }finally{
      setMarkingExternalId(null)
    }
  }
  ...
  const exportCSV = async ()=>{
    try{ setExporting(true)
      ...
      const header=["公司名称","岗位名称","薪资","地点","经验","学历","HR","投递状态","AI打招呼","申请路由","外部申请状态","申请链接","行业","公司规模","发布时间","创建时间"]
      const rows = all.map(it=>[
        it.companyName||"", it.jobName||"", it.salary||"", it.location||"", it.experience||"", it.degree||"", it.hrName||"", it.deliveryStatus||"", it.greetingStatus||"未处理", it.applicationRoute||"INTERNAL", it.externalApplyStatus||"", it.applicationUrl||it.jobUrl||"", it.industry||"", it.companyScale||"", it.publishTime||"", it.createdAt||""
      ])
  ...
  const kpiCards = useMemo(()=>{ const k=stats?.kpi; return [ { title:"总岗位数", value:k?.total??0 }, { title:"已投递", value:k?.delivered??0 }, { title:"未投递", value:k?.pending??0 }, { title:"待外部申请", value:k?.externalPending??0 }, { title:"外部已申请", value:k?.externalApplied??0 }, { title:"平均月薪(K)", value:k?.avgMonthlyK??0 } ] },[stats])

  return (
    <div className="space-y-8">
      {showHeader && (
        <PageHeader title="51job 投递分析" subtitle="基于 job51_data 表的统计图与列表分析" icon={<ChartBar size={28} />} />
      )}
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mt-4 flex gap-3">
            <Button onClick={async()=>{ await loadList(1,size); await loadStats() }} disabled={loadingList}><ChartBar className="mr-2" /> 应用筛选</Button>
            <Button variant="success" onClick={exportCSV} disabled={exporting}><Download className="mr-2" /> {exporting?"导出中...":"导出CSV"}</Button>
            <Button variant="outline" onClick={onReload} disabled={reloading}><RefreshCw className="mr-2" /> 刷新数据</Button>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><PieChart /> 投递状态分布</CardTitle><CardDescription>已投递/未投递占比</CardDescription></CardHeader><CardContent>{stats?(<ChartSurface type="pie" labels={stats.charts.byStatus.map(x=>x.name)} data={stats.charts.byStatus.map(x=>x.value)} />):(<div className="h-64 flex items-center justify-center border-2 border-dashed rounded-lg text-muted-foreground">加载中...</div>)}</CardContent></Card>
        <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><ChartBar /> 行业TOP10</CardTitle><CardDescription>岗位按行业聚合</CardDescription></CardHeader><CardContent>{stats?(<ChartSurface type="bar" labels={stats.charts.byIndustry.map(x=>x.name)} data={stats.charts.byIndustry.map(x=>x.value)} colors={CATEGORY_COLORS} />):(<div className="h-64 flex items-center justify-center border-2 border-dashed rounded-lg text-muted-foreground">加载中...</div>)}</CardContent></Card>
        <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><ChartBar /> 公司岗位数TOP10</CardTitle><CardDescription>按公司名称聚合</CardDescription></CardHeader><CardContent>{stats?(<ChartSurface type="bar" labels={stats.charts.byCompany.map(x=>x.name)} data={stats.charts.byCompany.map(x=>x.value)} colors={CATEGORY_COLORS} />):(<div className="h-64 flex items-center justify-center border-2 border-dashed rounded-lg text-muted-foreground">加载中...</div>)}</CardContent></Card>
        <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><ChartBar /> 经验分布</CardTitle><CardDescription>不同经验要求的岗位数</CardDescription></CardHeader><CardContent>{stats?(<ChartSurface type="bar" labels={stats.charts.byExperience.map(x=>x.name)} data={stats.charts.byExperience.map(x=>x.value)} colors={CATEGORY_COLORS} />):(<div className="h-64 flex items-center justify-center border-2 border-dashed rounded-lg text-muted-foreground">加载中...</div>)}</CardContent></Card>
        <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><ChartBar /> 学历分布</CardTitle><CardDescription>不同学历要求的岗位数</CardDescription></CardHeader><CardContent>{stats?(<ChartSurface type="bar" labels={stats.charts.byDegree.map(x=>x.name)} data={stats.charts.byDegree.map(x=>x.value)} colors={CATEGORY_COLORS} />):(<div className="h-64 flex items-center justify-center border-2 border-dashed rounded-lg text-muted-foreground">加载中...</div>)}</CardContent></Card>
        <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><LineChart /> 薪资区间分布</CardTitle><CardDescription>基于中位数K的桶聚合</CardDescription></CardHeader><CardContent>{stats?(<ChartSurface type="line" labels={stats.charts.salaryBuckets.map(x=>x.bucket)} data={stats.charts.salaryBuckets.map(x=>x.value)} color="#ef4444" />):(<div className="h-64 flex items-center justify-center border-2 border-dashed rounded-lg text-muted-foreground">加载中...</div>)}</CardContent></Card>
      </div>

      {/* 列表 */}
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2"><Briefcase /> 岗位数据</CardTitle>
        ...
        <CardContent>
          <div className="overflow-x-auto rounded-xl border border-stroke/30 dark:border-strokedark/30 shadow-lg">
            <table className="w-full table-fixed min-w-[1280px] bg-white dark:bg-blacksection">
              <thead>
                <tr className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 border-b-2 border-blue-200 dark:border-blue-800">
                  ...
                  <th className="w-28 px-4 py-3.5 text-left text-sm font-semibold">AI打招呼</th>
                  <th className="w-16 px-4 py-3.5 text-left text-sm font-semibold">链接</th>
                  <th className="w-40 px-4 py-3.5 text-left text-sm font-semibold">外部申请</th>
              ...
              <tbody>
                {items.length===0 ? (
                  <tr><td colSpan={15} className="px-4 py-12 text-center text-muted-foreground bg-gray-50 dark:bg-gray-900/20"><div className="flex flex-col items-center gap-3"><Briefcase className="text-4xl text-gray-300 dark:text-gray-600" /><p className="text-sm">暂无数据</p></div></td></tr>
                ) : (
                  items.map((it,idx)=> (
                    <tr key={it.jobId} className={`group transition-colors border-b border-gray-200 dark:border-gray-700 last:border-b-0 ${idx%2===0?'bg-white dark:bg-blacksection hover:bg-blue-50/50 dark:hover:bg-blue-950/20':'bg-gray-50/50 dark:bg-gray-900/20 hover:bg-blue-50/50 dark:hover:bg-blue-950/20'}`}>
                      ...
                      <td className="px-4 py-3 text-sm leading-6 whitespace-nowrap align-top"><span title={it.greetingError||it.greetingTime||''} className={`px-2 py-1 rounded-full text-xs ${it.greetingStatus === 'SENT' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : it.greetingStatus === 'FAILED' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' : it.greetingStatus === 'SKIPPED' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300' : 'bg-slate-100 text-slate-700 dark:bg-slate-800/50 dark:text-slate-300'}`}>{it.greetingStatus || '未处理'}</span></td>
                      <td className="px-4 py-3 text-sm leading-6 whitespace-nowrap align-top">{(it.applicationUrl || it.jobUrl)? (<a href={it.applicationUrl || it.jobUrl} className="text-primary underline" target="_blank" rel="noreferrer">链接</a>):('-')}</td>
                      <td className="px-4 py-3 text-sm leading-6 align-top">
                        {it.applicationRoute === 'EXTERNAL' ? (
                          <div className="flex flex-col items-start gap-1">
                            <a href={it.applicationUrl || it.jobUrl} className="inline-flex items-center gap-1 text-primary underline" target="_blank" rel="noreferrer">
                              <ExternalLink className="h-3.5 w-3.5" /> 打开申请页
                            </a>
                            {it.externalApplyStatus === 'APPLIED' ? (
                              <span className="text-xs text-green-700 dark:text-green-300">已申请</span>
                            ) : (
                              <Button variant="outline" size="sm" onClick={() => void markExternalApplied(it.jobId)} disabled={markingExternalId === it.jobId}>
                                <CheckCircle2 className="mr-1 h-3.5 w-3.5" />
                                {markingExternalId === it.jobId ? '处理中...' : '标记已申请'}
                              </Button>
                            )}
                          </div>
                        ) : '-'}
