...
import com.jobradar.worker.utils.Job;
...
public class ZhiLian {
    ...
    private boolean deliverCurrentPage(String keyword) {
        try {
            ...
            for (PageJob pj : jobs) {
                ...
                ZhilianJobDataEntity persistedJob = zhilianService.findJobById(pj.jobId);
                funnelMark("collected", pj.jobId);
                if (normalizeJobLink(pj.jobLink) != null) funnelMark("detail_link", pj.jobId);
                if (shouldSkipPersistedJob(persistedJob)) {
                    sendProgress(String.format("跳过已过滤岗位：%s", pj.jobTitle), null, null);
                    continue;
                }
                if (isExternalApplicationRoute(pj.jobLink)) {
                    funnelMark("external_pending", pj.jobId);
                    continue;
                }
                if (config.isAiScreeningEnabled() && !passesAiScreening(keyword, pj, persistedJob)) {
                    continue;
                }
                ...
                if (applyBtn.count() == 0) {
                    funnelMark("retryable", pj.jobId, "button_not_visible");
                    continue;
                }
                funnelMark("button_visible", pj.jobId);
                try {
                    ...
                    try {
                        if (pj.jobId != null && !pj.jobId.isEmpty()) {
                            boolean updated = zhilianService.markDeliveredByJobId(pj.jobId);
                            if (updated) {
                                funnelMark("formal_apply_success", pj.jobId);
                            } else {
                                funnelMark("retryable", pj.jobId, "delivery_state_not_updated");
                            }
                            log.info("已标记投递：jobId={}，title={}，company={}", pj.jobId, pj.jobTitle, pj.companyName);
                        } else if (pj.jobTitle != null && pj.companyName != null) {
                            boolean updated = zhilianService.markDeliveredByTitleAndCompany(pj.jobTitle, pj.companyName);
                            if (!updated) funnelMark("retryable", pj.jobId, "delivery_state_not_updated");
                            log.info("已标记投递：title={}，company={}", pj.jobTitle, pj.companyName);
                        }
                    } catch (Exception ex) {
                        funnelMark("retryable", pj.jobId, "delivery_state_update_failed");
                        log.warn("更新投递状态失败: {}", ex.getMessage());
                    }
                } catch (Exception clickEx) {
                    funnelMark("retryable", pj.jobId, "click_failed");
            ...
            try {
                saveCurrentPageHtml();
                log.info("已保存当前页面到 src/main/java/com/jobradar/worker/zhilian/page.html 以便排查");
    ...
    private boolean passesAiScreening(String keyword, PageJob job, ZhilianJobDataEntity persistedJob) {
        Page detailPage = null;
        try {
            String jd = usableCachedJobDescription(persistedJob);
            if (jd != null) {
                log.info("复用智联岗位JD缓存 jobId={} length={}", job.jobId, jd.length());
                if (persistedJob.getJobId() != null && !jd.equals(persistedJob.getJobDescription())) {
                    if (!zhilianService.saveJobDescription(persistedJob.getJobId(), jd)) {
                        return deferAiScreening(job, "缓存JD保存失败");
                    }
                }
                if (persistedJob != null && persistedJob.getJobId() != null) {
                    funnelMark("jd", persistedJob.getJobId());
                }
            } else {
                String link = normalizeJobLink(job.jobLink);
                if (link == null) return deferAiScreening(job, "缺少岗位详情链接");
                detailPage = page.context().newPage();
                detailPage.setDefaultTimeout(10_000);
                detailPage.navigate(link, new Page.NavigateOptions().setTimeout(10_000));
                PlaywrightUtil.sleep(1);
                jd = JobDescriptionExtractor.extractFromPage(detailPage,
                        "div.job-detail", "div.job-detail__content", "div[class*='job-detail']",
                        "div[class*='job-description']", "div[class*='job-intro']");
                if (!JobDescriptionExtractor.isUsable(jd)) {
                    return deferAiScreening(job, "未读取到可用JD");
                }
                if (!zhilianService.saveJobDescription(job.jobId, jd)) {
                    return deferAiScreening(job, "JD保存失败");
                }
            }
            AiEntity aiConfig = aiService.getAiConfig();
            JSONObject jobJson = new JSONObject();
            jobJson.put("jobId", valueOrEmpty(job.jobId));
            jobJson.put("title", valueOrEmpty(job.jobTitle));
            jobJson.put("company", valueOrEmpty(job.companyName));
            jobJson.put("salary", valueOrEmpty(job.salary));
            jobJson.put("area", valueOrEmpty(job.location));
            jobJson.put("education", valueOrEmpty(job.degree));
            jobJson.put("experience", valueOrEmpty(job.experience));
            jobJson.put("jd", jd);

            Map<String, String> values = new HashMap<>();
            values.put("candidate", aiConfig == null ? "" : valueOrEmpty(aiConfig.getIntroduce()));
            values.put("keyword", valueOrEmpty(keyword));
            values.put("min_score", String.valueOf(config.effectiveAiMinScore()));
            values.put("jobs", "[" + jobJson + "]");
            String raw = aiService.sendStructuredRequest(aiService.renderScreeningPrompt(
                    aiConfig == null ? null : aiConfig.getScreenPrompt(),
                    aiConfig == null ? null : aiConfig.getJdAnalysisPrompt(), values), 0.1, 256);
            LiepinAiBatchAssessment.Result parsed = LiepinAiBatchAssessment.parse(raw);
            LiepinAiBatchAssessment.Item assessment = parsed == null ? null : parsed.items().stream()
                    .filter(item -> valueOrEmpty(job.jobId).equals(item.jobId()))
                    .findFirst().orElse(null);
            if (assessment == null) {
                return deferAiScreening(job, "AI 响应无效或未包含当前岗位");
            }
            funnelMark("ai_valid", job.jobId);
            if (shouldPersistAiRejection(assessment, config.effectiveAiMinScore())) {
                return filterAiScreening(job, assessment.score() + "分，" + assessment.reason());
            }
            if (!isAccepted(assessment, config.effectiveAiMinScore())) {
                funnelMark("review", job.jobId);
                return deferAiScreening(job, "AI 结果需要复核");
            }
            funnelMark("ai_pass", job.jobId);
            sendProgress(String.format("AI JD分析通过：%s（%d分）", job.jobTitle, assessment.score()), null, null);
            return true;
        } catch (Exception e) {
            log.warn("智联 AI JD分析失败 jobId={} title={} reason={}",
                    job.jobId, job.jobTitle, e.getMessage());
            return deferAiScreening(job, "AI JD分析失败");
        } finally {
            if (detailPage != null) {
    ...
    static boolean shouldSkipPersistedJob(ZhilianJobDataEntity job) {
        return job != null && "已过滤".equals(job.getDeliveryStatus());
    }

    static String usableCachedJobDescription(ZhilianJobDataEntity job) {
        if (job == null) return null;
        String jd = JobDescriptionExtractor.normalize(job.getJobDescription());
        return JobDescriptionExtractor.isUsable(jd) ? jd : null;
    }

    private boolean filterAiScreening(PageJob job, String reason) {
        zhilianService.markFilteredByJobId(job.jobId);
        sendProgress(String.format("AI JD分析过滤：%s（%s）", job.jobTitle, reason), null, null);
        return false;
    }

    private boolean deferAiScreening(PageJob job, String reason) {
        sendProgress(String.format("AI JD分析暂缓：%s（%s）", job.jobTitle, reason), null, null);
        if (reason != null && reason.contains("复核")) {
            funnelMark("review", job.jobId, reason);
        } else {
            funnelMark("retryable", job.jobId, reason);
        }
        return false;
    }

    static boolean isAccepted(LiepinAiBatchAssessment.Item assessment, int minimumScore) {
        return assessment != null
                && assessment.score() >= Math.max(0, Math.min(100, minimumScore))
                && "PASS".equals(assessment.decision())
                && !assessment.reasonCodes().contains("HARD_MISMATCH")
                && !assessment.reason().contains("HARD_MISMATCH");
    }

    static boolean shouldPersistAiRejection(LiepinAiBatchAssessment.Item assessment, int minimumScore) {
        if (assessment == null) return false;
        if (assessment.reasonCodes().contains("HARD_MISMATCH")
                || assessment.reason().contains("HARD_MISMATCH")) return true;
    ...
    private String normalizeJobLink(String link) {
        ...
        if (value.startsWith("/")) return "https://www.zhaopin.com" + value;
    ...
    private boolean isExternalApplicationRoute(String link) {
        ...
        try {
            ...
            return host != null && !host.equalsIgnoreCase("zhaopin.com")
                    && !host.toLowerCase(java.util.Locale.ROOT).endsWith(".zhaopin.com");
        } catch (Exception ignored) {
            return false;
        }
    }
    ...
    private void funnelMark(String stage, String jobId) {
        funnelMark(stage, jobId, null);
    }

    private void funnelMark(String stage, String jobId, String reason) {
        if (jobFunnelService == null || jobId == null || jobId.isBlank()) return;
        switch (stage) {
            case "collected" -> jobFunnelService.collected("zhilian", jobId);
            case "detail_link" -> jobFunnelService.detailLink("zhilian", jobId);
            case "jd" -> jobFunnelService.jd("zhilian", jobId);
            case "ai_valid" -> jobFunnelService.aiValid("zhilian", jobId);
            case "ai_pass" -> jobFunnelService.aiPass("zhilian", jobId);
            case "review" -> jobFunnelService.review("zhilian", jobId, reason);
            case "button_visible" -> jobFunnelService.buttonVisible("zhilian", jobId);
            case "chat_success" -> jobFunnelService.chatSuccess("zhilian", jobId);
            case "formal_apply_success" -> jobFunnelService.formalApplySuccess("zhilian", jobId);
            case "external_pending" -> jobFunnelService.externalPending("zhilian", jobId);
            case "retryable" -> jobFunnelService.retryable("zhilian", jobId, reason);
            default -> { }
        }
    }
    ...
    private void saveCurrentPageHtml() {
        try {
            ...
            java.nio.file.Path path = java.nio.file.Paths.get("src/main/java/com/jobradar/worker/zhilian/page.html");
