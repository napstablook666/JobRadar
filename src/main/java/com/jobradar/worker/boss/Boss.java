...
import static com.jobradar.worker.boss.Locators.*;


/**
 * @author loks666
 * 项目链接: <a href=
 * "https://github.com/napstablook666/JobRadar">https://github.com/napstablook666/JobRadar</a>
 * Boss直聘自动投递
 */
@Slf4j
@Component
...
public class Boss {
        ...
        for (String keyword : config.getKeywords()) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (encryptId != null && !encryptId.isBlank()) {
                    job.setHref("https://www.zhipin.com/job_detail/" + encryptId + ".html");
                }

                if (Boolean.TRUE.equals(config.getEnableAiScreening())
                        && !Boolean.TRUE.equals(config.getDebugger())) {
                    if (job.getJobInfo() == null || job.getJobInfo().isBlank()) {
                        progressCallback.accept("AI筛选跳过：岗位缺少JD | " + job.getJobName(), i + 1, count);
                        continue;
                    }
                    LiepinAiBatchAssessment.Item assessment = screenJob(keyword, job);
                    if (!passesAiScreening(assessment, config.effectiveAiMinScore())) {
                        String reason = assessment == null ? "结果无效或请求失败" : assessment.reason();
                        progressCallback.accept("AI筛选跳过：" + job.getJobName() + " | " + reason, i + 1, count);
                        continue;
                    }
                    progressCallback.accept("AI筛选通过：" + job.getJobName()
                            + "（" + assessment.score() + "分）", i + 1, count);
                }
    ...
    private void processJobDetailJsonAndInsert(String body) {
        ...
        try {
            ...
            com.jobradar.application.entity.BossJobDataEntity entity = new com.jobradar.application.entity.BossJobDataEntity();
    ...
    private void resumeSubmission(String keyword, Job job) {
        ...
        // 5. 图片简历先发，避免文字消息抢在附件前到达
        boolean imageRequired = Boolean.TRUE.equals(config.getSendImgResume());
        boolean imgResume = !imageRequired || sendImageResume(detailPage);
        if (imageRequired && !imgResume) {
            log.warn("图片简历未确认发送，继续发送文字并将图片结果单独记录: {}", job.getJobName());
        }
        ...
        String message = isValidString(aiMessage) ? aiMessage : safeMessage(config.getSayHi());
    ...
    static boolean passesAiScreening(LiepinAiBatchAssessment.Item item, int minimumScore) {
        if (item == null || item.score() < Math.max(0, Math.min(100, minimumScore))) {
            return false;
        }
        if (!"PASS".equals(item.decision())) {
            return false;
        }
        return !item.reasonCodes().contains("HARD_MISMATCH")
                && !item.reason().contains("HARD_MISMATCH");
    }

    private LiepinAiBatchAssessment.Item screenJob(String keyword, Job job) {
        String jobId = aiJobId(job);
        try {
            AiEntity aiConfig = aiService.getAiConfig();
            String introduce = aiConfig == null || aiConfig.getIntroduce() == null
                    ? "" : aiConfig.getIntroduce();
            JSONObject jobJson = new JSONObject();
            jobJson.put("jobId", jobId);
            jobJson.put("jobName", safeMessage(job.getJobName()));
            jobJson.put("companyName", safeMessage(job.getCompanyName()));
            jobJson.put("salary", safeMessage(job.getSalary()));
            jobJson.put("jobArea", safeMessage(job.getJobArea()));
            jobJson.put("jobDescription", safeMessage(job.getJobInfo()));

            Map<String, String> values = new HashMap<>();
            values.put("candidate", introduce);
            values.put("keyword", safeMessage(keyword));
            ...
            log.warn("Boss AI筛选请求失败: job={}, error={}", job.getJobName(), e.getMessage());
            return null;
        }
    }

    private String aiJobId(Job job) {
        if (job != null && job.getHref() != null && !job.getHref().isBlank()) {
            return job.getHref();
        }
        return safeMessage(job == null ? null : job.getCompanyName())
                + "|" + safeMessage(job == null ? null : job.getJobName());
    }
    ...
    private boolean sendImageResume(Page page) {
        try {
            ...
            // Boss 的图片 input 通常隐藏在工具栏里，aria-label 不是固定字段。
            Locator imgContainer = page.locator(
                    ".chat-controls div.btn-sendimg, .btn-sendimg, .btn-send-img, .send-img, "
                            + "[class*='sendimg'], [class*='send-img']"
            ).first();
            Locator imageInput = page.locator(
                    ".chat-controls div.btn-sendimg input[type='file'], "
                            + ".btn-sendimg input[type='file'], "
                            + ".btn-send-img input[type='file'], "
                            + ".send-img input[type='file'], "
                            + ".toolbar input[type='file'], "
                            + "[class*='sendimg'] input[type='file'], "
                            + "[class*='send-img'] input[type='file'], "
                            + "input[type='file'][accept*='image']"
            ).first();
            int beforeImageMessages = countOutgoingImageMessages(page);
            ...
            if (imageInput.count() == 0) {
                ...
                if (imgContainer.count() > 0) {
                    ...
                    if (!fileSubmitted) {
                        PlaywrightUtil.sleep(1);
                        imageInput = imgContainer.locator("input[type='file']").first();
                    }
                }
            }
            if (!fileSubmitted) {
            ...
            // 某些版本会在选图后显示预览层，需要再次点击发送。
            Locator previewSend = page.locator(
                    ".image-wrap:visible .btn-send, .image-wrap:visible button[class*='send'], "
                            + ".image-preview:visible .btn-send, .image-preview:visible button[class*='send'], "
                            + ".preview-wrap:visible .btn-send, .preview-wrap:visible button[class*='send'], "
                            + "[class*='image-wrap']:visible .btn-send, "
                            + "[class*='image-preview']:visible .btn-send, "
                            + "[class*='preview-wrap']:visible .btn-send"
            ).first();
            if (previewSend.count() > 0 && previewSend.isVisible()) {
                previewSend.click();
            }
            return waitForOutgoingImage(page, beforeImageMessages, 15_000L);
        } catch (Throwable e) {
    ...
    private boolean waitForOutgoingImage(Page page, int beforeCount, long timeoutMillis) {
        long deadline = System.currentTimeMillis() + timeoutMillis;
        while (System.currentTimeMillis() < deadline) {
            if (countOutgoingImageMessages(page) > beforeCount) {
                return true;
            }
            try {
    ...
    private int countOutgoingImageMessages(Page page) {
        try {
            int exact = page.locator(
                    ".chat-conversation .message-item.item-myself .text.item-image "
                            + ".message-image-content img.message-image"
            ).count();
            if (exact > 0) {
                return exact;
            }
            return page.locator(
                    ".chat-conversation .message-item.item-myself img, "
                            + ".chat-conversation [class*='message-self'] img"
            ).count();
        } catch (Exception e) {
            return 0;
        }
    ...
    private String generateAiMessage(String keyword, String jobName, String jd) {
        ...
        try {
            String requestMessage = (prompt != null)
                    ? aiService.renderPrompt(prompt, introduce, keyword, jobName, jd, config.getSayHi())
                    : buildDefaultPrompt(introduce, keyword, jobName, jd);
            String result = aiService.sendRequest(requestMessage);
            if (result == null) {
                return config.getSayHi();
            }
            AiService.GreetingValidation validation = AiService.validateGreeting(result, introduce);
            return validation.usable() ? validation.message() : config.getSayHi();
        } catch (Exception e) {
