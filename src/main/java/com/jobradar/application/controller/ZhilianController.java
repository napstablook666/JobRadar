...
import com.jobradar.worker.service.ZhilianJobService;
...
public class ZhilianController {
    ...
    /** Read the persisted ten-stage funnel for Zhilian. */
    @GetMapping("/funnel")
    public ResponseEntity<Map<String, Object>> getFunnel() {
        return ResponseEntity.ok(jobFunnelService.getFunnel("zhilian"));
    }
    ...
    public ResponseEntity<Map<String, Object>> startZhilianJob() {
        ...
        try {
            ...
            boolean started = zhilianJobService.startDeliveryAsync(progressMessage ->
    ...
    public ResponseEntity<Map<String, Object>> stopZhilianJob() {
        ...
        try {
            ...
            playwrightManager.closePlatformBrowsersAfterStop("zhilian", zhilianJobService::isRunning);

            response.put("success", true);
            response.put("message", "智联招聘任务停止请求已发送，浏览器将在任务收尾后自动关闭");
