...
import com.jobradar.worker.service.BossJobService;
...
public class BossController {
    ...
    public ResponseEntity<Map<String, Object>> executeBoss() {
        ...
        boolean started = bossJobService.startDeliveryAsync(this::sendBossProgress);
        if (!started) {
            return ResponseEntity.ok(Map.of(
                    "status", "already_running",
                    "message", "Boss投递任务已在运行中"
    ...
    public ResponseEntity<Map<String, Object>> startBoss() {
        ...
        try {
            ...
            boolean started = bossJobService.startDeliveryAsync(pm -> {
                sendBossProgress(pm);
    ...
    public ResponseEntity<Map<String, Object>> stopBoss() {
        ...
        try {
            ...
            playwrightManager.closePlatformBrowsersAfterStop("boss", bossJobService::isRunning);
            response.put("success", true);
            response.put("message", "Boss任务停止请求已发送，浏览器将在任务收尾后自动关闭");
    ...
    public ResponseEntity<Map<String, Object>> logoutBoss() {
        Map<String, Object> response = new HashMap<>();
        try {
            // 先清 Cookie，再标记未登录（Boss 改为 Cookie 登录，不再 hands-off）
