...
function ChartCanvas({
  ...
  async function ensureChart(): Promise<ChartConstructor> {
    ...
    return new Promise<ChartConstructor>((resolve, reject) => {
      ...
      if (existing) {
        existing.addEventListener("load", () => window.Chart ? resolve(window.Chart) : reject(new Error("Chart.js global missing")))
      ...
      script.addEventListener("load", () => window.Chart ? resolve(window.Chart) : reject(new Error("Chart.js global missing")))
...
export default function AnalysisContent({ showHeader = false }: { showHeader?: boolean }) {
  ...
  // 默认先看真实已投递记录；取消“已投递”后仍可切换回完整岗位池。
  const [statuses, setStatuses] = useState<string[]>(["已投递"])
