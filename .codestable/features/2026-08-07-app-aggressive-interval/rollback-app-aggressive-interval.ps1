...
'@
    $rollbackSource = $rollbackSource.Replace([string][char]0xFEFF, '').TrimStart([char]0x0D, [char]0x0A)
    $scriptFile = Join-Path $env:TEMP ("get-jobs-rollback-" + [guid]::NewGuid().ToString('N') + '.jsh')
    try {
        [IO.File]::WriteAllText($scriptFile, $rollbackSource, (New-Object System.Text.UTF8Encoding($false)))
        $rollbackOutput = & jshell --class-path $jar -q $scriptFile 2>&1
        $rollbackExitCode = $LASTEXITCODE
        $rollbackOutput
        if ($rollbackExitCode -ne 0 -or -not ($rollbackOutput -match 'ROLLBACK_DB_OK')) {
            throw "Database rollback failed with exit code $rollbackExitCode"
        }
    } finally {
