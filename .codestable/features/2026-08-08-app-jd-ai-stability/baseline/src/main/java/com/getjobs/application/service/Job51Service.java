...
public class Job51Service {
    ...
    private void ensureJob51ConfigSchema() {
        try (Connection conn = dataSource.getConnection(); Statement stmt = conn.createStatement()) {
            ...
            try { stmt.execute("ALTER TABLE job51_config ADD COLUMN max_per_run INTEGER NOT NULL DEFAULT 10"); } catch (Exception ignored) {}
            try { stmt.execute("ALTER TABLE job51_config ADD COLUMN min_delay_seconds INTEGER NOT NULL DEFAULT 5"); } catch (Exception ignored) {}
            try { stmt.execute("ALTER TABLE job51_config ADD COLUMN max_delay_seconds INTEGER NOT NULL DEFAULT 10"); } catch (Exception ignored) {}
        } catch (Exception e) {
