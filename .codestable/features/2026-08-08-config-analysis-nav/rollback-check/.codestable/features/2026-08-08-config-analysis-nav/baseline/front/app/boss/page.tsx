...
import { Briefcase, Save, Search, Banknote, Building2, Trash2, Plus, Play, Square, LogOut, KeyRound, ChevronDown } from 'lucide-react'
...
const AnalysisContent = dynamic(() => import('@/app/boss/analysis/AnalysisContent'), {
  ssr: false,
...
export default function BossPage() {
  ...
  const [cookieImportResult, setCookieImportResult] = useState<{ success: boolean; message: string } | null>(null)
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- setup is intentionally mount-only
  }, [])
  ...
  const fetchAllData = async () => {
    try {
      ...
      if (data.config) {
        ...
        setKeywordsDisplay(keywordLinesForDisplay(data.config.keywords))
        // 解析括号列表为数组
  ...
  const handleSave = async (silent: boolean = false, overrides?: Partial<BossConfig>) => {
    try {
      ...
      const payload: BossConfig = {
        ...
        ...(overrides || {}),
        keywords: keywordLinesForStorage(keywordsDisplay),
  ...
  const handleImportCookie = async () => {
    ...
    if (!raw) {
      setCookieImportResult({ success: false, message: '请先粘贴 Cookie 内容' })
      return
    }
    setImportingCookie(true)
    setCookieImportResult(null)
    try {
      const response = await fetch('http://localhost:8888/api/cookie/import', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ platform: 'boss', cookie_value: raw, remark: 'boss cookie login' }),
      })
      const data = await response.json()
      if (data.success) {
        ...
        setCookieImportResult({ success: false, message: data.message || '导入失败' })
      }
    } catch (error) {
      console.error('Failed to import cookie:', error)
      setCookieImportResult({ success: false, message: '导入失败：网络或服务异常' })
    } finally {
        ...
        <TabsContent value="config" className="space-y-6 mt-6">
          ...
          <Card className="animate-in fade-in slide-in-from-bottom-5 duration-700">
            <CardHeader>
              ...
              <CardDescription>
                {isLoggedIn ? '当前已登录' : '自动化浏览器内登录易被拦截，请改用 Cookie 导入'}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <ol className="list-decimal list-inside space-y-1 text-sm text-muted-foreground">
                  <li>用系统 Chrome 打开 <span className="font-mono">https://www.zhipin.com</span> 并完成登录</li>
                  <li>F12 → Application/存储 → Cookies → 复制，或使用 EditThisCookie 导出 JSON</li>
                  <li>粘贴到下方（支持 JSON 数组 或 <span className="font-mono">name=value; a=b</span>）并点击导入</li>
                </ol>
                <div className="space-y-2">
                  <Label htmlFor="boss-cookie">Cookie 内容</Label>
                  <Textarea
                    id="boss-cookie"
                    value={cookieInput}
                    onChange={(e) => setCookieInput(e.target.value)}
                    placeholder='[{"name":"wt2","value":"...","domain":".zhipin.com","path":"/"}] 或 wt2=xxx; __zp_stoken__=yyy'
                    className="min-h-[120px] font-mono text-xs"
                    disabled={importingCookie}
                  />
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <Button
                    ...
                    className="rounded-full bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white px-4"
                  >
                    <KeyRound className="mr-1" />
                    {importingCookie ? '导入中...' : '导入 Cookie 登录'}
                  </Button>
                  {cookieImportResult && (
                    <span className={`text-sm ${cookieImportResult.success ? 'text-green-600' : 'text-red-500'}`}>
                      {cookieImportResult.message}
                    </span>
                  )}
                </div>
                <p className="text-sm text-muted-foreground">登录成功后可点「开始投递」。Cookie 会写入数据库，下次启动自动注入。</p>
              </div>
            </CardContent>
          </Card>
  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      >
        ...
        <ChevronDown className={`ml-2 h-4 w-4 shrink-0 text-muted-foreground transition-transform duration-200 ${open ? 'rotate-180' : ''}`} aria-hidden="true" />
      </button>
          ...
          {enableSearch && (
            <div className="dropdown-search sticky top-0 z-10 border-b border-black/5 bg-white/95 p-2 dark:border-white/10 dark:bg-neutral-900/95">
              <input
                ...
                className="h-8 w-full rounded-full border border-black/10 bg-white/80 px-3 text-sm outline-none ring-0 placeholder:text-muted-foreground focus:border-cyan-300/70 focus:ring-2 focus:ring-cyan-400/30 dark:border-white/15 dark:bg-white/5"
              />
            </div>
          )}
          <div className="dropdown-panel-list flex flex-col gap-2 p-2">
            {filteredOptions.length === 0 ? (
              <div className="px-3 py-3 text-center text-sm text-muted-foreground">无匹配选项</div>
            ) : (
                ...
                return (
                  ...
                  >
                    <span className="flex items-center gap-3 min-w-0">
                      <span className={`inline-flex h-4 w-4 shrink-0 items-center justify-center rounded-md border border-white/30 bg-white/10 shadow-inner transition-all ${checked ? 'bg-teal-400/60 border-teal-300/80' : ''}`}></span>
