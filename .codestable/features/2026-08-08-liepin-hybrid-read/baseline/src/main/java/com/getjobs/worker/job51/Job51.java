...
public class Job51 {
    ...
    private AiJobOutcome processAiJob(String keyword, Job51Entity entity, String company, String title) {
        ...
        try {
            ...
            if (!navigateDetailPage(detailPage, entity.getJobId(), link)) {
                String reason = checkAccessVerification(detailPage) || checkNeedLogin(detailPage)
                        ? "detail_blocked" : "detail_navigation_failed";
                markGreetingSkipped(entity.getJobId(), reason);
                sendProgress(String.format("跳过%s：详情页加载失败（%s）", title, reason), null, null);
                return AiJobOutcome.SKIPPED;
            }

            JobDescriptionResult description = resolveJobDescription(detailPage, entity.getJobDescription());
            ...
            markGreetingSkipped(entity.getJobId(), "job_processing_error");
    ...
    private boolean navigateDetailPage(Page detailPage, Long jobId, String storedLink) {
        for (String link : detailLinkCandidates(jobId, storedLink)) {
            try {
                ...
                if (checkAccessVerification(detailPage) || checkNeedLogin(detailPage)) {
                    log.warn("[51job] 详情页出现登录或访问验证 job_id={} link={}", jobId, link);
                    continue;
                }
                return true;
            } catch (Exception e) {
    ...
    private String extractMarkedDescription(Page detailPage) {
        try {
            ...
            String[] starts = {"职位描述", "岗位职责", "工作内容"};
            String[] ends = {"任职要求", "任职资格", "职位要求", "福利待遇", "公司信息"};
            int start = firstMarkerIndex(body, starts);
            if (start < 0) return null;
            int contentStart = start;
            int end = firstMarkerIndex(body, ends, start + 1);
            String section = end > contentStart ? body.substring(contentStart, end) : body.substring(contentStart);
            return normalizeJobDescription(section);
        } catch (Exception ignored) {
            return null;
        }
    }
    ...
    private int firstMarkerIndex(String text, String[] markers) {
        return firstMarkerIndex(text, markers, 0);
    }

    private int firstMarkerIndex(String text, String[] markers, int fromIndex) {
        int result = -1;
    ...
    private boolean isUsableJobDescription(String text) {
        return text != null && text.length() >= MIN_JOB_DESCRIPTION_LENGTH
                && !text.contains("访问验证") && !text.contains("请按住滑块")
                && !text.equals("登录");
    }
            ...
            for (int attempt = 1; attempt <= 2; attempt++) {
                ...
                try {
                    ...
                    if (classified.rejected()) {
                        return new AiGreetingResult(null, true, "ai_rejected", attempt);
                    }
                    ...
                    if (attempt == 1 && shouldRetryAiReason(lastReason)) {
                        waitForAiRetry();
                        continue;
                    }
                    break;
                    ...
                    if (attempt == 1 && isRetryableAiFailure(e)) {
                        waitForAiRetry();
                        continue;
                    }
                    break;
    ...
    private boolean shouldRetryAiReason(String reason) {
        return reason != null
                && !reason.equals("ai_rejected")
                && !reason.equals("ai_prompt_error")
                && !reason.equals("ai_prompt_missing");
    }

    private boolean isRetryableAiFailure(Throwable error) {
        String message = messageOf(error, "").toLowerCase(Locale.ROOT);
        return !message.contains("状态码: 400")
                && !message.contains("状态码: 401")
                && !message.contains("状态码: 403")
                && !message.contains("状态码: 404")
                && !message.contains("状态码: 422");
    }

    private String classifyAiFailure(Throwable error) {
        String message = messageOf(error, "").toLowerCase(Locale.ROOT);
        if (message.contains("timed out") || message.contains("timeout")) return "ai_timeout";
        if (message.contains("状态码: 400") || message.contains("状态码: 401")
                || message.contains("状态码: 403") || message.contains("状态码: 404")
                || message.contains("状态码: 422")) return "ai_request_rejected";
        return "ai_request_error";
    }

    private void waitForAiRetry() {
        try {
            Thread.sleep(300L);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    private boolean checkAccessVerification(Page target) {
        try {
            ...
            Locator verifyText = target.locator("text=访问验证, text=请按住滑块");
