        ...
        <Card className="animate-in fade-in slide-in-from-bottom-6 duration-700">
          ...
          <CardContent>
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="autoAiDelivery">AI筛选自动投递</Label>
                <label className="flex h-10 items-center gap-3 rounded-md border px-3 text-sm">
                  <input
                    id="autoAiDelivery"
                    type="checkbox"
                    disabled={config.enableAi === 0}
                    checked={config.autoAiDelivery === 1}
                    onChange={(e) => setConfig({ ...config, autoAiDelivery: e.target.checked ? 1 : 0 })}
                  />
                  <span>{config.autoAiDelivery === 1 ? '已开启，评分达标自动发送' : '已关闭，逐岗位等待确认'}</span>
                </label>
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={0}
                  max={100}
                  disabled={config.autoAiDelivery !== 1}
