...
import { BiSearch, BiSave, BiTargetLock, BiMoney, BiPlay, BiStop, BiLogOut, BiBriefcase, BiCheck, BiX, BiRefresh } from 'react-icons/bi'
...
type TaskState = 'IDLE' | 'RUNNING' | 'STOPPING' | 'PAUSED' | 'COMPLETED' | 'FAILED'
...
export default function LiepinPage() {
  const [config, setConfig] = useState<LiepinConfig>({
    ...
    searchMaxDelaySeconds: 20,
    pageMinDelaySeconds: 5,
    pageMaxDelaySeconds: 10,
    detailMinDelaySeconds: 8,
    detailMaxDelaySeconds: 15,
    rateGuardBatchSize: 5,
    batchCooldownMinSeconds: 300,
    batchCooldownMaxSeconds: 450,
  })
  ...
  const fetchAllData = async () => {
    try {
      ...
      if (data.config) {
        ...
        normalized.minDelaySeconds = Math.max(data.config.minDelaySeconds ?? 30, 30)
        normalized.maxDelaySeconds = Math.max(data.config.maxDelaySeconds ?? 60, normalized.minDelaySeconds, 60)
        normalized.searchMinDelaySeconds = Math.max(5, Math.min(300, data.config.searchMinDelaySeconds ?? 10))
        normalized.searchMaxDelaySeconds = Math.max(normalized.searchMinDelaySeconds,
          Math.min(300, Math.max(10, data.config.searchMaxDelaySeconds ?? 20)))
        normalized.pageMinDelaySeconds = Math.max(5, Math.min(300, data.config.pageMinDelaySeconds ?? 5))
        normalized.pageMaxDelaySeconds = Math.max(normalized.pageMinDelaySeconds,
          Math.min(300, Math.max(5, data.config.pageMaxDelaySeconds ?? 10)))
        normalized.detailMinDelaySeconds = Math.max(5, Math.min(300, data.config.detailMinDelaySeconds ?? 8))
        normalized.detailMaxDelaySeconds = Math.max(normalized.detailMinDelaySeconds,
          Math.min(300, Math.max(8, data.config.detailMaxDelaySeconds ?? 15)))
        normalized.rateGuardBatchSize = Math.max(1, Math.min(10, data.config.rateGuardBatchSize ?? 5))
        normalized.batchCooldownMinSeconds = Math.max(300, Math.min(3600, data.config.batchCooldownMinSeconds ?? 300))
        normalized.batchCooldownMaxSeconds = Math.max(normalized.batchCooldownMinSeconds,
          Math.min(3600, Math.max(300, data.config.batchCooldownMaxSeconds ?? 450)))
  ...
  const refreshTaskStatus = async () => {
    try {
      ...
      const responseRunId = Number(data.runId || 0)
      ...
      setTaskState((data.taskState || 'IDLE') as TaskState)
      ...
      setAiSummary({ ...EMPTY_AI_SUMMARY, ...(data.aiSummary || {}) })
      setDeliverySummary({ ...EMPTY_DELIVERY_SUMMARY, ...(data.deliverySummary || {}) })
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (disposed) return
      timer = window.setTimeout(poll, data?.taskState === 'STOPPING' ? 250 : data?.isRunning ? 1000 : 3000)
    }
  ...
  const handleRetryAiTimeouts = async () => {
    try {
      setIsDelivering(true)
      setTaskMessage('正在重试 AI 超时岗位...')
      setTaskMessageType('info')
      const response = await fetch('http://localhost:8888/api/liepin/retry-ai', {
        method: 'POST',
      })
      const data = await response.json()
      if (!data.success) {
        setIsDelivering(false)
        setTaskMessage(data.message || '启动 AI 重试失败。')
        setTaskMessageType('error')
        return
      }
      const startedRunId = Number(data.runId || 0)
      ...
      setTaskMessage(data.message || 'AI 超时岗位重试已启动。')
      setTaskMessageType('info')
      await refreshTaskStatus()
    } catch (error) {
      console.error('[猎聘] 启动 AI 超时重试失败:', error)
      setIsDelivering(false)
      setTaskMessage('启动 AI 重试失败：网络或服务异常。')
      setTaskMessageType('error')
    }
  }
          ...
          <div className="flex items-center gap-2">
            {checkingLogin ? (
              ...
              <Button onClick={handleStopDelivery} disabled={taskState === 'STOPPING'} size="sm" className="rounded-full bg-gradient-to-r from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 disabled:opacity-70 disabled:cursor-wait text-white px-4 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                <BiStop className="mr-1" /> {taskState === 'STOPPING' ? '停止中...' : '停止投递'}
              </Button>
            ) : (
              <div className="flex items-center gap-2">
                ...
                {(aiSummary.aiRetryable ?? 0) > 0 && (
                  <Button onClick={handleRetryAiTimeouts} size="sm" variant="outline" className="rounded-full border-amber-400 text-amber-700 hover:bg-amber-50 px-4">
                    <BiRefresh className="mr-1" /> 重试 AI 超时 ({aiSummary.aiRetryable})
                  </Button>
                )}
              </div>
            )}
      ...
      <Tabs defaultValue="config" className="w-full">
        ...
        <Card className="animate-in fade-in slide-in-from-bottom-6 duration-700">
          <CardHeader>
            ...
            <CardDescription>按月薪 K 筛选，网页年薪档位会自动换算</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 gap-6">
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">薪资范围码（例如：6$10 表示 6K-10K；岗位区间有交集即进入后续筛选）</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={30}
                  max={300}
                  value={config.maxDelaySeconds ?? 60}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={5}
                  max={300}
                  value={config.searchMinDelaySeconds ?? 10}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={5}
                  max={300}
                  value={config.searchMaxDelaySeconds ?? 20}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={5}
                  max={300}
                  value={config.pageMinDelaySeconds ?? 5}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={5}
                  max={300}
                  value={config.pageMaxDelaySeconds ?? 10}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={5}
                  max={300}
                  value={config.detailMinDelaySeconds ?? 8}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={5}
                  max={300}
                  value={config.detailMaxDelaySeconds ?? 15}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={300}
                  max={3600}
                  value={config.batchCooldownMinSeconds ?? 300}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={300}
                  max={3600}
                  value={config.batchCooldownMaxSeconds ?? 450}
                  onChange={(e) => setConfig({ ...config, batchCooldownMaxSeconds: Number(e.target.value) })}
                />
              </div>
            </div>
            <p className="mt-3 text-xs text-muted-foreground">
              当前极限提速基线：搜索 10-20 秒、翻页 5-10 秒、详情 8-15 秒、发送 30-60 秒；命中平台风控信号后任务会停止并记录原因。
            </p>
        ...
        <TabsContent value="analytics" className="space-y-6 mt-6">
          <AnalysisContent deliveryRunning={isDelivering} />
        </TabsContent>
      </Tabs>
