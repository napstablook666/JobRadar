...
Start-Process -FilePath "cmd.exe" -ArgumentList @("/c", "call", "`"$FrontRun`"") -WorkingDirectory $Front -WindowStyle Normal
...
Start-Process -FilePath "cmd.exe" -ArgumentList @("/c", "call", "`"$BackRun`"") -WorkingDirectory $Root -WindowStyle Normal

$ready = $false
for ($i = 1; $i -le 120; $i++) {
    if (Test-HttpOk "http://127.0.0.1:8888/") {
        Write-Step "[OK] backend ready (~${i}s) log: $BackLog"
        $ready = $true
        break
    }
    Start-Sleep -Seconds 1
